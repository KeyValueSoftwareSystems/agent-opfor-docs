...
| `attackObjective`      | No                            | Free-text primary mission steering every evaluator's attacks. Also settable via `--objective`/`--objective-file` on `opfor run` |
| `judgeHint`            | No                            | Free-text steering for the judge's verdict. Combined with each attack's existing judge hint rather than replacing it. Also settable via `--judge-hint`/`--judge-hint-file` |
| `businessUseCase`      | No                            | Free-text domain/business context for the target agent. Also settable via `--business-use-case`/`--business-use-case-file` |
