...
<Note>`safetyScore` and `attackSuccessRate` are **severity-weighted** — each attack's contribution is scaled by its evaluator's severity (critical = 4, high = 3, medium = 2, low = 1), so critical failures dominate the headline scores. The raw `passed` / `failed` / `errors` counts stay unweighted for transparency.</Note>
